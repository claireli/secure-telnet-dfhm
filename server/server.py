...
class TelnetSecureServer:
  ...
  def broadcast(self, message, sender_id, login_event=False):
    ...
    for client in self.client_threads:
      ...
      client.send(f'{self.client_sessions[id]}{EOT}'.encode('utf-8'))
  ...
  def client_thread(self, client, client_ip):
    ...
    while alive:
        ...
        if message:
            print('INCOMING DATA', message, type(message), len(message))
            print('CONVERT BYTES TO ?', message.decode())
            #self.broadcast(message.decode('utf-8'), id)
